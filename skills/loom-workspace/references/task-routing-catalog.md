...
| --- | --- | --- |
| "Fix this bug", failing test, broken build, regression, flaky behavior, incident, unexpected error | Is the failure reproduced and is root cause known? | `loom-debugging` -> evidence -> `loom-tickets` -> local execution or `loom-ralph` |
...
| Performance problem, slow path, bundle size, Core Web Vitals, high traffic concern | Is there a baseline measurement? | `loom-evidence` baseline -> `loom-research` or `loom-spike` -> ticket -> after evidence -> performance critique |
...
| UI page, component, styling, responsive layout, accessibility, visual polish | Is primary user task and quality bar clear? | `loom-specs` if fuzzy -> `loom-spike` for variants when needed -> ticket -> visual/product evidence -> critique |
...
| Architecture improvement, module deepening, codebase more testable, tangled dependency | Is this investigation, decision, or execution? | `loom-codemap`/`loom-research` -> `loom-specs` or `loom-constitution` for decisions -> plan/ticket |
| Database schema, storage, data migration, import/export, persistence safety | Does order, rollback, or idempotency matter? | `loom-plans` when sequencing matters -> ticket with `data-migration` -> before/after evidence -> critique |
...
| "Grill me", stress-test this idea/plan, refine idea, ideate, challenge assumptions | Is the output a behavior contract, investigation, sequencing decision, or accepted explanation? | `loom-workspace` problem shaping -> `loom-research`, `loom-specs`, `loom-plans`, or `loom-wiki` by owner truth |
| Prototype this, try a few designs, sanity-check a state model, let me play with it | What question should the throwaway artifact answer? | `loom-spike` -> evidence/research -> spec/wiki/ticket route after conclusion |
...
| "Where is X?", unfamiliar code, understand this module, map architecture | Is durable orientation useful for future agents? | `loom-codemap` -> evidence/research -> wiki atlas when accepted |
| PR description, changelog, release note, launch, rollback, monitoring, handoff, merge package | Are tickets/evidence/critique already truthful? | `loom-ship`; route back to tickets/evidence/critique if not truthful |
| "Is this done?", "close it", "ready to merge?", acceptance or residual risk check | Does ticket-owned acceptance support closure? | `loom-tickets` acceptance gate; `loom-ship` only after truth is current |
| User asks to keep going on a broad outcome | Is there delegated objective, success criteria, and stop conditions? | `loom-drive` -> owner layers -> tickets/Ralph/evidence/critique |
