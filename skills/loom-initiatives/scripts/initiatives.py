...
RECORD_DIRECTORY = Path(".loom/initiatives")
...
def initiative_records(workspace: Path) -> list[Path]:
...
def read_initiative(path: Path) -> tuple[dict, str]:
...
def resolve_initiative_target(workspace: Path, target: str) -> Path:
    ...
    for path in initiative_records(workspace):
        frontmatter, _body = read_initiative(path)
        if frontmatter.get("id") == expected_id:
            return path
    raise SystemExit(f"Unknown initiative target: {target}")


def create_initiative(args: argparse.Namespace) -> int:
...
def link_initiative(args: argparse.Namespace) -> int:
    if not args.add and not args.remove:
        raise SystemExit("Provide at least one --add or --remove argument")
    workspace = find_workspace_root()
    record_path = resolve_initiative_target(workspace, args.target)
    frontmatter, body = read_initiative(record_path)
    links = frontmatter.get("links", {})
    if not isinstance(links, dict):
        links = {}
    normalized = parse_link_arguments([])
...
def main() -> int:
    parser = argparse.ArgumentParser(
        prog="initiatives.py", description="Initiative CLI"
    )
    subparsers = parser.add_subparsers(dest="command")

    create_parser = subparsers.add_parser("create", help="Create an initiative record")
    ...
    create_parser.set_defaults(func=create_initiative)

    link_parser = subparsers.add_parser("link", help="Add or remove initiative links")
    ...
    link_parser.set_defaults(func=link_initiative)
