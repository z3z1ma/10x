...
def constitution_main_path(workspace: Path) -> Path:
    return workspace / ".loom/constitution/constitution.md"


def decision_directory(workspace: Path) -> Path:
    return workspace / ".loom/constitution/decisions"


def roadmap_directory(workspace: Path) -> Path:
    return workspace / ".loom/constitution/roadmap"


def next_decision_number(workspace: Path) -> int:
    highest = 0
    directory = decision_directory(workspace)
    ...
    for path in directory.glob("*.md"):
        ...
        if isinstance(record_id, str):
            match = re.match(r"^decision:(\d+)$", record_id)
            if match:
...
def create_constitution(args: argparse.Namespace) -> int:
    ...
    if args.kind == "constitution":
        ...
        if not args.slug:
            raise SystemExit("decision creation requires a slug")
        number = next_decision_number(workspace)
        slug = slugify(args.slug)
        path = decision_directory(workspace) / f"decision-{number:04d}-{slug}.md"
        frontmatter = {
            "id": f"decision:{number:04d}",
            "kind": "decision",
            "schema_version": 1,
            "status": args.status or "active",
        ...
        if not args.slug:
            raise SystemExit("roadmap creation requires a slug")
        slug = slugify(args.slug)
        path = roadmap_directory(workspace) / f"{slug}.md"
        if path.exists():
            raise SystemExit(
                f"Refusing to overwrite existing record: {relative_to_workspace(path, workspace)}"
            )
...
def link_constitution(args: argparse.Namespace) -> int:
    ...
    if candidate.is_absolute() and candidate.exists():
        ...
        workspace_candidate = (workspace / args.target).resolve()
        if workspace_candidate.exists():
            path = workspace_candidate
        else:
            raise SystemExit(f"Unknown constitutional target: {args.target}")
    frontmatter, body = parse_frontmatter(path.read_text())
    if frontmatter.get("id") == "constitution:main" and args.add:
        raise SystemExit("constitution:main must remain link-free")
    ...
    for key, values in parse_links(args.remove).items():
        ...
        if not normalized[key]:
            normalized.pop(key)
    if frontmatter.get("id") == "constitution:main" and normalized:
        raise SystemExit("constitution:main must remain link-free")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        prog="constitution.py", description="Constitution CLI"
    )
    subparsers = parser.add_subparsers(dest="command")

    create_parser = subparsers.add_parser(
        "create", help="Create constitution, decision, or roadmap records"
    )
    create_parser.add_argument("kind", choices=["constitution", "decision", "roadmap"])
    ...
    create_parser.set_defaults(func=create_constitution)

    link_parser = subparsers.add_parser(
        "link", help="Add or remove decision and roadmap links"
    )
    ...
    link_parser.set_defaults(func=link_constitution)
