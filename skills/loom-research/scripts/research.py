...
RECORD_DIRECTORY = Path(".loom/research")
...
def research_records(workspace: Path) -> list[Path]:
...
def read_research(path: Path) -> tuple[dict, str]:
...
def resolve_research_target(workspace: Path, target: str) -> Path:
    ...
    for path in research_records(workspace):
        frontmatter, _body = read_research(path)
        if frontmatter.get("id") == expected_id:
            return path
    raise SystemExit(f"Unknown research target: {target}")
...
def link_research(args: argparse.Namespace) -> int:
    if not args.add and not args.remove:
        raise SystemExit("Provide at least one --add or --remove argument")
    workspace = find_workspace_root()
    record_path = resolve_research_target(workspace, args.target)
    frontmatter, body = read_research(record_path)
...
def main() -> int:
    parser = argparse.ArgumentParser(prog="research.py", description="Research CLI")
    subparsers = parser.add_subparsers(dest="command")

    create_parser = subparsers.add_parser("create", help="Create a research record")
    ...
    create_parser.set_defaults(func=create_research)

    link_parser = subparsers.add_parser("link", help="Add or remove research links")
    ...
    link_parser.set_defaults(func=link_research)
