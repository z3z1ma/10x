...
RECORD_KIND = "spec"
RECORD_DIRECTORY = Path(".loom/specs")
...
def spec_records(workspace: Path) -> list[Path]:
...
def resolve_spec_target(workspace: Path, target: str) -> Path:
    ...
    for path in spec_records(workspace):
        frontmatter, _body = read_spec(path)
        if frontmatter.get("id") == expected_id:
            return path
    raise SystemExit(f"Unknown spec target: {target}")


def create_spec(args: argparse.Namespace) -> int:
...
def link_spec(args: argparse.Namespace) -> int:
    if not args.add and not args.remove:
        raise SystemExit("Provide at least one --add or --remove argument")
    workspace = find_workspace_root()
    record_path = resolve_spec_target(workspace, args.target)
    frontmatter, body = read_spec(record_path)
...
def main() -> int:
    parser = argparse.ArgumentParser(prog="specs.py", description="Spec CLI")
    subparsers = parser.add_subparsers(dest="command")

    create_parser = subparsers.add_parser("create", help="Create a spec record")
    ...
    create_parser.set_defaults(func=create_spec)

    link_parser = subparsers.add_parser("link", help="Add or remove spec links")
    ...
    link_parser.set_defaults(func=link_spec)
