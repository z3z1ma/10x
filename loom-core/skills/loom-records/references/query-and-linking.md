...
queries against the installed skills directory for the current harness. In this
split source checkout those roots are `loom-core/skills` and
`loom-playbooks/skills`; in an installed bundle they may be package caches or
adapter-specific skill paths. Do not assume a target Loom workspace has a
...
Use the actual skills installation path for the current harness. In this split
source checkout this may be `loom-core/skills` or `loom-playbooks/skills`; in an
installed bundle it may be a package cache or adapter-specific skill path.
