...
This reference names optional playbook skills when they are useful routes. If a
named playbook is not installed, keep operating through the core owner layers,
local execution, Ralph, or a project-provided equivalent workflow instead of
treating the missing playbook as a core Loom blocker.
...
- high-level objective continuation through owner records, ticket tranches,
  ...
  optional `loom-drive` or a project-provided objective driver
...
- implementation isolation, branch/worktree hygiene, or Git provenance ->
  optional `loom-git` support coordinator or project Git practice
- debugging or incident flow -> optional `loom-debugging` or equivalent investigation workflow
- bounded experiment, prototype, or sketch -> optional `loom-spike` or equivalent exploration workflow
- codebase/module atlas work -> optional `loom-codemap` or equivalent atlas workflow
- merge, release, PR, or handoff packaging from already-truthful owner records ->
  optional `loom-ship` or equivalent shipping workflow
...
When installed, `loom-ship` packages PR summaries, release notes, handoff packages,
