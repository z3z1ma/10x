...
- **compound-cli-change-requires-contract-tests** (74%) [cli, compound, contracts, determinism, testing]
  - Trigger: When editing src/agent_loom/compound/cli.py (especially large refactors/simplifications) or behavior-adjacent modules like src/agent_loom/compound/sync.py
  - Action: Treat CLI output + generated artifacts as a contract: ensure deterministic ordering/formatting, update or add focused pytest coverage (prefer existing contract tests like tests/test_compound_install.p...
