...
Legacy note: Risk — high - changes the session-start doctrine and preload surfaces that govern all Loom routing.

Priority: high - largest context-impact slice after the compression contract.
Depends-On: .loom/tickets/done/20260525-compression-contract-inventory.md
...
- `.loom/specs/loom-protocol-compression.md` - defines compression requirements, especially REQ-001 through REQ-003.
- `.loom/decisions/project-constitution.md` - requires operational kernels and portability.
- `roadmap:loom-mill` - names protocol compression as the foundation chapter.
- `.loom/specs/ticket-owned-worker-handoffs.md` - worker handoff behavior that session doctrine must preserve.
- `.loom/tickets/done/20260525-compression-contract-inventory.md` - provides inventory and baseline.
...
- ACC-002: The compressed startup doctrine still teaches the complete loop: shape, route durable truth, slice to tickets, execute bounded Ralph runs, preserve evidence, audit claims, and reconcile records.
  - Evidence: Source inspection against `.loom/specs/loom-protocol-compression.md#REQ-001` and `REQ-003`.
...
Validation evidence: `.loom/evidence/20260525-session-kernel-compression-validation.md` records before/after line counts, Core smoke, Core pack check, `git diff --check`, targeted behavior searches, and product-surface leakage search. Acceptance is satisfied by implementation evidence and fresh-context audit.
...
- 2026-05-25: Bounded source-edit run compressed `using-loom` from 955 to 722 lines and the requested preload set from 1494 to 1261 lines. Validation passed: Core smoke, Core pack check, `git diff --check`, targeted behavior/source searches, and product-surface leakage search. Recorded evidence at `.loom/evidence/20260525-session-kernel-compression-validation.md`; moved ticket to `review` pending fresh-context audit.
- 2026-05-25: Fresh-context audit recorded at `.loom/reviews/20260525-session-kernel-compression-audit.md`. Verdict: pass with non-blocking risks; no material findings. Audit reran Core smoke, Core pack check, and `git diff --check`, inspected the compressed source, preload surfaces, validation evidence, and product-surface leakage searches.
