...
Legacy note: Risk — medium - needs proper DAG layout algorithm with correct ontology ordering and connected-nodes-only scope

## Summary
The current "Hierarchy" mode in the graph view is a naive layered layout that shows all workspace records and uses incorrect layer ordering. It needs to be rebuilt as a proper DAG view:
1. **Only show connected nodes** - like the connected graph mode, start from the current record and expand to connected records. Not a full-workspace dump.
2. **Correct ontology ordering**: research precedes anything (it informs all other surfaces), specs precede plans (specs define behavior that plans decompose into work), audits are children of tickets (like evidence).
3. **True DAG layout** - not a simple "group by layer" grid. Use actual DAG layout algorithms (Sugiyama/layered or force-directed with layer constraints) that respect edge direction and minimize crossings. Can be top-down or left-right.
...
- `.loom/tickets/20260526-mill-graph-dag.md` - previous naive implementation to replace
...
## Current State
Closed. Implemented Sugiyama-lite layout algorithm with barycenter heuristic for crossing minimization. Updated ontology ordering and removed the scope dropdown.

## Journal
- 2026-05-26: Created ticket. Replaces naive hierarchy from .loom/tickets/20260526-mill-graph-dag.md.
- 2026-05-26: Implemented proper DAG layout in GraphView.svelte. Verified with Playwright screenshots. Build passes.
