...
Legacy note: Risk — medium - changes Core session-entry doctrine and package-visible activation guidance, but remains prose/static-check work.

## Summary

Implement the activation lessons from `.loom/research/20260513-superpowers-skill-activation.md` in Loom's shipped surfaces. The closure claim is that Core now uses the Superpowers-style OpenCode bootstrap path, teaches near-explicit Superpowers activation language, preserves anti-rationalization pressure, and ships tests that check actual skill invocation.

## Related Records

- `.loom/research/20260513-superpowers-skill-activation.md` - source investigation and recommendations this ticket implements.
- `.loom/research/20260510-loom-loop-failure-analysis.md` - prior analysis of premature execution and worker-handoff bypasses.
- `.loom/tickets/done/20260513-mandatory-shaping-doctrine.md` - recent closed ticket strengthening ambiguity shaping doctrine.
...
- `.loom/evidence/20260513-superpowers-style-activation-checks.md` - source scans, Core smoke, Playbooks smoke, package dry-runs, and diff check supporting the acceptance criteria.
...
Evidence `.loom/evidence/20260513-superpowers-style-activation-checks.md` supports the acceptance criteria. Separate audit was not performed because the criteria are adapter/static-text/test-output claims with direct command evidence.
...
- 2026-05-13: Recorded `.loom/evidence/20260513-superpowers-style-activation-checks.md` after source scans, Core smoke, Playbooks smoke, Core pack check, Playbooks pack check, and `git diff --check` passed. Closed the ticket with no separate audit per the acceptance audit posture.
