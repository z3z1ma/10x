...
Legacy note: Risk — medium - moves a shared Playbook package path consumed by OpenCode, Claude, Cursor, Codex, Gemini, package files, and contributor guidance.
...
- `.loom/specs/playbook-explicit-macros.md` - requires Gemini command macros and no Playbook skill exposure.
- `.loom/tickets/done/20260515-playbook-macro-catalog.md` - closed prerequisite that created the catalog seam.
- `.loom/tickets/done/20260515-opencode-playbook-commands.md` - closed ticket whose OpenCode catalog reader must be updated to the relocated path.
- `.loom/tickets/done/20260515-native-playbook-explicit-surfaces.md` - closed ticket whose native plugin manifests and explicit-only metadata must follow the relocated path.
- `.loom/tickets/done/20260515-gemini-playbook-commands.md` - blocked consumer that requires top-level `skills/` removal before command TOML work can honestly proceed.
...
Must not implement Gemini `commands/*.toml` in this ticket. That remains `.loom/tickets/done/20260515-gemini-playbook-commands.md` after relocation closes.
