...
Legacy note: Risk — medium - changes Claude, Cursor, and Codex plugin-facing Playbook behavior and must avoid overclaiming unsupported Codex commands.
...
- `.loom/tickets/20260515-playbook-explicit-macros.md` - defines sequencing and validation posture for this adapter unit.
- `.loom/tickets/done/20260515-playbook-macro-catalog.md` - provides the canonical macro source this ticket should consume.
- `.loom/specs/playbook-explicit-macros.md` - defines explicit-only requirements for Claude, Cursor, and Codex.
- `.loom/research/20260515-playbook-command-surfaces.md` - records command and explicit-only skill support by harness.
- `.loom/reviews/20260515-native-playbook-explicit-surfaces.md` - clear Ralph-backed review of ACC-001 through ACC-005.
...
Codex must use explicit-only plugin skills with `policy.allow_implicit_invocation: false` unless source-backed research added after `.loom/research/20260515-playbook-command-surfaces.md` proves plugin-contributed prompt commands.
...
- 2026-05-15: Set status to active and launched `former packet 20260515T220601Z-native-playbook-explicit-surfaces` after the catalog ticket closed.
- 2026-05-15: `former packet 20260515T220601Z-native-playbook-explicit-surfaces` returned `stop`: added Claude/Cursor explicit-only frontmatter to all 25 Playbooks, added Codex per-skill `agents/openai.yaml` explicit-only policies, tightened native plugin metadata, validated the Claude plugin, and ran `git diff --check`. Moved ticket to review because audit/acceptance review is the next closure step.
- 2026-05-15: Recorded clear Ralph-backed audit in `.loom/reviews/20260515-native-playbook-explicit-surfaces.md` and closed the ticket with runtime-validation limits explicit.
