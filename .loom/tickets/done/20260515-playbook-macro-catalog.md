...
Legacy note: Risk — medium - establishes the shared source that later adapter packaging will consume.
...
- `.loom/tickets/20260515-playbook-explicit-macros.md` - decomposes the broader Playbook conversion and makes this the prerequisite unit.
- `.loom/specs/playbook-explicit-macros.md` - defines explicit macro behavior and evidence expectations.
- `.loom/research/20260515-playbook-command-surfaces.md` - constrains adapter targets that will consume the catalog.
- `.loom/research/20260515-playbooks-core-activation-pressure.md` - explains why the macro framing must avoid automatic activation pressure.
- `.loom/reviews/20260515-playbook-macro-catalog.md` - clear Ralph-backed review of ACC-001 through ACC-005.
...
Closed. Ralph packet `former packet 20260515T213657Z-playbook-macro-catalog` added a derived macro catalog seam in `loom-playbooks/loom-playbooks.mjs`: `readPlaybookMacroCatalog()` wraps every current Playbook `SKILL.md` body with explicit optional macro framing, and `inspectPlaybookMacroCatalog()` verifies the generated catalog has one macro per Playbook with non-trigger-style catalog descriptions. Evidence gathered: Playbooks smoke passed with `skillCount: 25`, `macroCount: 25`, no missing macros, and no automatic description failures; package introspection showed the skill and macro name sets match exactly; representative macro inspection showed explicit framing for idea refinement, debugging, UI, source-driven development, incremental implementation, security, and release lenses; `git diff --check` passed.

Audit `.loom/reviews/20260515-playbook-macro-catalog.md` returned `clear` with no material findings. ACC-001 through ACC-005 are satisfied for the catalog-only closure claim. Residual risk is intentionally routed to downstream adapter tickets: they must consume `readPlaybookMacroCatalog().body` instead of raw `SKILL.md` bodies, and OpenCode still registers `skills/` until `.loom/tickets/done/20260515-opencode-playbook-commands.md` runs.

## Journal

- 2026-05-15: Created ticket from `.loom/tickets/20260515-playbook-explicit-macros.md`; adapter tickets are blocked on this shared catalog decision.
- 2026-05-15: Set status to active and launched `former packet 20260515T213657Z-playbook-macro-catalog` for the canonical macro catalog implementation slice.
- 2026-05-15: `former packet 20260515T213657Z-playbook-macro-catalog` returned `stop`: implemented a derived explicit macro catalog seam from the existing Playbook skill corpus, ran `npm --prefix loom-playbooks run smoke`, confirmed the macro set matches the 25-skill Playbook set, inspected representative macro framing, and ran `git diff --check`. Moved ticket to review because audit/acceptance review is the next closure step.
- 2026-05-15: Recorded clear Ralph-backed audit in `.loom/reviews/20260515-playbook-macro-catalog.md` and closed the ticket. Adapter implementation tickets are unblocked.
