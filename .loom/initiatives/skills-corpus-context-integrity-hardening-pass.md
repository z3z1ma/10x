...
- Complete third-pass precision follow-ups for route readiness, drive priority,
  workspace routing, placeholder hygiene, source metadata, packet safety,
  problem shaping, README support framing, and related workflow boundaries.
...
- OBJ-013: Ticket route readiness covers every legal route token that a ready
  ticket may name.
- OBJ-014: Drive tranche priority includes `continue` when the current owner
  already names the next bounded tranche.
- OBJ-015: Drive tranche priority distinguishes wiki capture from retrospective
  compounding.
- OBJ-016: Workspace routing rows consistently separate route token from owner or
  coordinator skill.
- OBJ-017: Record validation flags unresolved placeholders in saved `.loom`
  project records without treating intentional template placeholders as failures.
- OBJ-018: Bootstrap file-creation examples are copy-safe and include a local
  placeholder check.
- OBJ-019: Research templates prompt for enough source metadata to re-evaluate
  external or current-source claims.
- OBJ-020: Shared packet frontmatter fails closed on ambiguous empty child write
  scopes.
- OBJ-021: Ralph launch checklists explicitly block unresolved placeholders and
  unauthorized `next_route` mismatches.
- OBJ-022: Orphan packet repair routes by packet family before downstream repair.
- OBJ-023: Problem shaping aligns ambiguous-choice guidance with the newer
  `ask_user` posture for low-risk reversible assumptions.
- OBJ-024: README explains workspace and harness metadata as support metadata.
- OBJ-025: Workspace doctor terminology does not label support-path checks as
  canonical owner presence.
- OBJ-026: Packet execution context treats `network: unknown` as a launch blocker
  unless justified.
- OBJ-027: Packet source fingerprints expose dirty Git state with
  machine-readable categories.
- OBJ-028: Retrospective read order includes memory when memory promotion or
  pruning is possible.
- OBJ-029: Critique packet template placeholders use safe quoted `<TBD: ...>`
  shapes consistently.
- OBJ-030: README route table is framed as introductory while route vocabulary
  owns complete saved-field grammar.
...
- Milestone 4: Local edit, query recipes, drive, and ship/acceptance boundaries
  close.
- Milestone 5: Third-pass precision follow-up tickets close.
- Milestone 6: Plan and initiative are accepted.
...
- Research: `research:skills-corpus-third-pass-follow-up-validation`
...
- Tickets: `ticket:bootinv1`, `ticket:trustbd2`, `ticket:vocabx08`,
  ...
  `ticket:shipacc1`, followed by `ticket:rready12`, `ticket:drvcont13`,
