...
class TestTicketSync(unittest.TestCase):
    def test_noop(self) -> None:
        with _temp_git_repo() as (root, env):
            env = {**env, "TICKET_DIR": str(root / ".loom" / "ticket")}
            (root / ".loom" / "ticket").mkdir(parents=True)
            (root / ".loom" / "ticket" / "a.md").write_text("# A\n", encoding="utf-8")
    ...
    def test_creates_commit(self) -> None:
        with _temp_git_repo() as (root, env):
            ...
            (root / ".loom" / "ticket" / "a.md").write_text(
                "# A\n\nupdated\n", encoding="utf-8"
            )
            code, payload = _ticket_json(
                ["--json", "--no-audit", "sync"], cwd=root, env=env
            ...
            self.assertEqual(payload.get("files"), [".loom/ticket/a.md"])
            ...
            self.assertEqual(
                [p for p in shown.splitlines() if p.strip()], [".loom/ticket/a.md"]
            )
    ...
    def test_commits_tickets_even_when_other_changes_present(self) -> None:
        with _temp_git_repo() as (root, env):
            ...
            self.assertNotIn(".loom/ticket/a.md", status)
