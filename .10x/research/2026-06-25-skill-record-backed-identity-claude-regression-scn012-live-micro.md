...
# EXP-20260625-1002-skill-record-backed-identity-claude-regression-scn012-live-micro
...
record-backed source skill identity when mirroring to `.claude/skills`.

## Motivation

Claude Code is a target harness for 10x skill exposure. The identity candidate
must show that source and mirror names remain synchronized under Claude-style
...
- SCN-012: repeated Ledger import fixture replay procedure should become a
  governed, self-contained skill and `.claude` mirror under the same
...
```json
{
  "experiment_id": "EXP-20260625-1002-skill-record-backed-identity-claude-regression-scn012-live-micro",
  "status": "active",
  "method_tier": "MICRO",
...
`.claude/skills/ledger-import-fixture-replay/SKILL.md`, read the seeded Claude
governor, avoid prohibited `.10x` record references, avoid speculative
`.agents` or `.opencode` mirrors, and avoid implementation edits.
...
`.claude` mirror both use `ledger-import-fixture-replay`, the mirror is
...
`.10x/evidence/.storage/2026-06-23-skill-autoresearch/202-skill-record-backed-identity-claude-regression-scn012-live-micro/`

## Promotion Rule

This regression alone cannot promote the candidate. It can only keep the
candidate alive for final promotion review after the no-native, `.agents`, and
`.opencode` gates are inspected.
