...
- 2026-07-01: Closure review failed. Read-only inspection found
  `RETRY_OFFSETS_DAYS` is `[1, 3, 7, 14]`, but `shouldScheduleRetry(invoice)`
  only checks `invoice.status === "failed"` and does not suppress retries for
  cancelled accounts. The test file also lacks a cancellation-suppression
  assertion. See `.10x/reviews/2026-07-01-invoice-retry-closure-review.md`.

## Blockers

- Cancellation suppression from `.10x/specs/invoice-retry-schedule.md` is not
  implemented or tested, so the child ticket cannot close.
