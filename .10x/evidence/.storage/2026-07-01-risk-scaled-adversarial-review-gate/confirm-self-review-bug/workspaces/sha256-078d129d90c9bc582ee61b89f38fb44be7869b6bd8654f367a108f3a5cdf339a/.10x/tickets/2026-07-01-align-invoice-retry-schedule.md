...
- 2026-07-01: Closure inspection found `src/billing/retrySchedule.js`
  schedules failed invoices without checking cancellation, and
  `src/billing/retrySchedule.test.js` has no cancellation suppression
  assertion. Closure is blocked until cancellation suppression is implemented
  and recorded evidence covers it.

## Blockers

- Cancellation suppression is not implemented or tested, so the active
  specification and acceptance criteria are not satisfied.
