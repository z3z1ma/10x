...
- 2026-07-01: Closure inspection found `src/billing/retrySchedule.js`
  returns true for any failed invoice and `src/billing/retrySchedule.test.js`
  does not assert cancellation suppression. Existing evidence therefore does
  not satisfy the cancellation acceptance criterion.

## Blockers

- Cancellation suppression is not implemented or tested according to parent
  inspection. `shouldScheduleRetry({ status: "failed", cancelled: true })`
  would still schedule a retry under the inspected implementation.
