...
- 2026-07-01: Closure inspection found the implementation and tests do not
  satisfy cancellation suppression from `.10x/specs/invoice-retry-schedule.md`.
  `src/billing/retrySchedule.js` schedules any failed invoice, and
  `src/billing/retrySchedule.test.js` does not assert that cancellation
  suppresses retry scheduling.

## Blockers

- Cancellation suppression is not implemented or tested, so this ticket cannot
  close against the active specification.
