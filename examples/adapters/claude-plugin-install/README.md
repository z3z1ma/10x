...
- that hook group has one command per `using-loom` reference; each command
  ...
  cats exactly one
  `${CLAUDE_PLUGIN_ROOT}/skills/using-loom/references/<filename>` file to stdout.
...
using-Loom context from per-reference `SessionStart` hook stdout.

This avoids three wrong solutions:

- custom agents as a substitute for `using-loom`
...
- the `SessionStart` hook emits the current ordered `using-loom` references as
  source-marked stdout outputs, one file per command
- same-session startup probes can see all current ordered using-Loom references
  without Claude's persisted-output preview/truncation behavior
- each current using-Loom reference remains below Claude's documented 10,000-character
...
- `${CLAUDE_PLUGIN_ROOT}` is the plugin installation/cache directory. Claude docs
  do not describe `${CLAUDE_PLUGIN_ROOT}/skills/using-loom/references` as a
...
- calling a Claude plugin install complete when `using-loom` is not discoverable
- relying on a plugin custom agent's prompt as the Loom operating layer
- concatenating all Loom using-Loom references into one hook output and accepting a
...
using-Loom context is emitted from bundled plugin files at session start.
