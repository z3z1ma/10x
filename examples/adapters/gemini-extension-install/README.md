...
- `contextFileName` points at `gemini-bootstrap.md`, which imports the ordered
  `skills/using-loom/references/*.md` files with Gemini's native `@file.md`
...
- `using-loom` remains a canonical skill under `skills/`.
- Bootstrap preload is an extension-native convenience, not a second doctrine
  source.
- The context file imports canonical using-Loom references instead of duplicating
  their content.
...
- copying using-Loom references into `~/.gemini/GEMINI.md` as the product install
  path
- generating Gemini command wrappers from removed top-level `commands/`
- treating `gemini-bootstrap.md` as more canonical than `skills/using-loom`
