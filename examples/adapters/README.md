...
- `using-loom` is discoverable, and its references are visible where the
...
Protocol semantics still live in `skills/`, `using-loom` references,
templates, and owner records.

Current fixtures:

- `claude-plugin-install/` - Claude native plugin with optional using-Loom preload
...
- `gemini-extension-install/` - Gemini CLI native extension with using-Loom preload
