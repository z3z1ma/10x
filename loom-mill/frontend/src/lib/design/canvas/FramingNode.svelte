<script lang="ts">
  ...
  let text = $derived(node.content.framing || node.content.text || 'No content');
</script>

<Node id={node.id} {position} let:selected>
  <div class="bg-bg-surface border border-l-4 border-l-purple-500 rounded-lg p-3 min-w-[220px] max-w-[320px] shadow-sm
    ...
    <div class="text-[11px] font-semibold text-purple-400 mb-1 uppercase tracking-wider">◆ Framing</div>
