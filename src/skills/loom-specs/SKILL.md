...
- `create_spec.py`: use when new behavior needs a durable contract in `.loom/specs/`
- `create_spec.py`: after running it, fill the body immediately; the script only creates the scaffold
- `link_records.py`: use to add or remove typed upstream and downstream links once the contract shape is real
- `validate_record.py`: use before handing the spec to plans, tickets, critique, or docs
