...
- `create_ticket.py`: use when a new ticket record needs to be created in `.loom/tickets/`
- `create_ticket.py`: after running it, fill the body immediately; the script only scaffolds the record
- `create_ticket.py`: prefer shorthand links like `--link ticket:0004` when you already know the related refs
- `link_records.py`: use to add or remove typed refs such as dependencies, verification, critique, and docs links after the body is in place
- `create_verification.py`: use as soon as the ticket gains real execution evidence that should participate in the durable graph
- `validate_record.py`: use before changing status or treating the ticket as ready, reviewable, or closed
- `check_links.py`: use to validate the record graph; this is a workspace-level link integrity check, not a single-file linter
- `resolve_scope.py`: use before broadening scope or assuming repository ownership
