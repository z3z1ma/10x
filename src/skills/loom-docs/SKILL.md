...
- `create_doc.py`: use when a new governed explanation is needed in `.loom/docs/`
- `create_doc.py`: after running it, populate the body immediately; the script only creates the shell
- `compile_packet.py`: use when docs work should be done through a fresh-context packetized pass
- `link_records.py`: use to add or remove truth-source and verification refs after the explanation is grounded
- `create_verification.py`: use when docs work or its supporting checks produce durable evidence
- `validate_record.py`: use before treating the doc as accepted enough to cite
- `check_links.py`: use to confirm truth-source and verification links resolve across the graph
- `resolve_scope.py`: use before editing across ambiguous repository or worktree boundaries
