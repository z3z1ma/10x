...
- `create_research.py`: use when a new question needs durable investigation space in `.loom/research/`
- `create_research.py`: after running it, populate the body immediately; the script only creates the record shell
- `link_records.py`: use after the note has meaningful conclusions so downstream consumers are explicit
- `validate_record.py`: use before handing the note to downstream work that will rely on it as evidence
