...
- `validate_memory_module.py`: validate manifest, required files, and L0 coverage
- `scan_memory_l0.py`: inspect the current memory surface quickly
- `rebuild_memory_glacier_index.py`: rewrite `.loom/memories/glacier/index.md` from archive frontmatter
- `rebuild_memory_link_index.py`: rewrite `.loom/memories/link-index.md` from `[[wiki-links]]`

## Manual Invocation

For explicit human-invoked housekeeping, use `src/commands/loom-memory-housekeeping.md`.
