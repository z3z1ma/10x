...
- `compile_packet.py`: use before launch to persist the Ralph contract under `.loom/runs/ralph/`
- `create_verification.py`: use after the run when the outcome or supporting checks should become durable evidence
- `validate_record.py`: use before and after execution when structural trust matters
- `check_links.py`: use to confirm the record graph stayed coherent after reconciliation
- `resolve_scope.py`: use whenever scope is unclear before packet compilation or launch
...
- `validate_record.py` passes and `check_links.py` does not surface unresolved graph problems after reconciliation
