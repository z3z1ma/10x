...
- `create_constitution.py`: use when the main constitution record itself must be seeded or intentionally replaced
- `create_constitution.py`: after running it, fill in the actual constitutional content immediately; the script only scaffolds the record and does not accept outbound links for `constitution:main`
- `create_decision.py`: use when one bounded decision needs its own durable record under `.loom/constitution/decisions/`
- `create_decision.py`: after running it, explain the decision, why it matters, and what it changes downstream
- `create_roadmap.py`: use when strategic direction needs roadmap framing rather than identity or one decision
- `create_roadmap.py`: after running it, populate milestones, scope, and strategic intent so the roadmap is operationally useful
- `link_records.py`: use after the prose is in place to add or remove typed downstream links on decision and roadmap records; `constitution:main` is implicitly linked to everything and should stay link-free
- `validate_record.py`: use before leaving the constitutional layer so structure, status, and required sections are trustworthy
