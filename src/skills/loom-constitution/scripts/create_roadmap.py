...
def main() -> int:
    parser = argparse.ArgumentParser(description="Create a Loom roadmap record")
    ...
    roadmap_slug = slugify(args.slug)
    path = create_record(
        "roadmap",
        args.slug,
        workspace,
        ...
        record_id=f"roadmap:{roadmap_slug}",
        file_slug=roadmap_slug,
    )
    if args.section:
        set_sections(
            workspace,
