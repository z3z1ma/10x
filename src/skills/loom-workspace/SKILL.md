...
1. start with `show_status.py` to summarize the current workspace state and locate likely owning surfaces
2. run `diagnose_workspace.py` before trusting downstream records, packets, or operator guidance
3. use `list_records.py` when you need canonical refs before linking, reviewing, or compiling packets
4. run `resolve_scope.py` for any path or repository ownership question and fail closed if it cannot assign scope cleanly
5. use `validate_record.py` and `check_links.py` before depending on a record graph that may have drifted
6. route into the owning subsystem only after the workspace is structurally trustworthy enough to proceed
7. if the workspace is not trustworthy, fix that first instead of pushing uncertainty into the next skill
...
- `diagnose_workspace.py`: use before depending on the workspace for packet work, review, or durable edits
