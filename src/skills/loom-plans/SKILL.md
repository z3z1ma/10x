...
- `create_plan.py`: use when execution strategy needs a durable home in `.loom/plans/`
- `create_plan.py`: after running it, populate the body immediately; the script only creates the scaffold
- `link_records.py`: use to add or remove linked constitution, initiative, spec, and ticket refs once the strategy is real
- `validate_record.py`: use before relying on the plan as live strategic guidance
