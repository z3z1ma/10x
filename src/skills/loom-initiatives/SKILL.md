...
- `create_initiative.py`: use when a new strategic outcome needs a durable record in `.loom/initiatives/`
- `create_initiative.py`: after running it, populate the strategy immediately; the script only scaffolds the record
- `link_records.py`: use to add or remove linked research, specs, plans, and tickets once the initiative body is real
- `validate_record.py`: use before relying on the initiative as strategic truth
