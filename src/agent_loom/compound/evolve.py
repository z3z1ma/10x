...
def _cluster_title(cluster: str) -> str:
    return " ".join(part.capitalize() for part in cluster.split("-") if part) or "General"


def _command_name_from_trigger(trigger: str, fallback: str) -> str:
    cmd_name = str(trigger or "").lower()
    cmd_name = cmd_name.replace("when ", "").replace("implementing ", "").replace("a ", "")
...
def _agent_markdown(name: str, instincts: list[Instinct]) -> str:
    ...
    lines.extend(["", "Do not deviate from these instincts without explicit user override.", ""])
    return "\n".join(lines)
...
def _skill_candidates(instincts: list[Instinct]) -> list[dict[str, Any]]:
    ...
    for cluster, cluster_items in by_cluster.items():
        if len(cluster_items) < 2:
            continue
        avg_conf = sum(float(i.confidence or 0.0) for i in cluster_items) / float(len(cluster_items))
        candidates.append(
            {
                "cluster": cluster,
                "instincts": sorted(cluster_items, key=lambda x: (-float(x.confidence or 0.0), x.id)),
                "avg_confidence": avg_conf,
                "domains": sorted({str(i.domain or "general") for i in cluster_items}),
            }
        )

    candidates.sort(key=lambda x: (-len(x["instincts"]), -float(x["avg_confidence"]), str(x["cluster"])))
    return candidates
...
def _agent_candidates(skill_candidates: list[dict[str, Any]]) -> list[dict[str, Any]]:
    out = [
        ...
        if len(list(c.get("instincts") or [])) >= 3 and float(c.get("avg_confidence") or 0.0) >= 0.75
    ]
    out.sort(key=lambda x: (-len(list(x.get("instincts") or [])), -float(x.get("avg_confidence") or 0.0), str(x.get("cluster") or "")))
    return out
...
def evolve_instincts(*, root: Path, threshold: float, generate: bool) -> EvolveResult:
    ...
    for skill in skill_candidates:
        ...
        _write(paths.evolved_skills_dir / f"{cluster}.md", _skill_markdown(cluster, instincts))
    ...
    for inst in command_candidates:
        ...
        _write(paths.evolved_commands_dir / f"instinct-{name}.md", _command_markdown(name, inst))

    for agent in agent_candidates:
        cluster = str(agent["cluster"])
        instincts = list(agent["instincts"])
        name = f"{cluster}-agent"
        _write(paths.evolved_agents_dir / f"instinct-{name}.md", _agent_markdown(name, instincts))
    ...
    if generate:
        for skill in skill_candidates:
            ...
            _write(repo / ".claude" / "skills" / "learned" / cluster / "SKILL.md", skill_text)
            _write(repo / ".opencode" / "skills" / "learned" / cluster / "SKILL.md", skill_text)
        ...
        for inst in command_candidates:
            ...
            _write(repo / ".opencode" / "commands" / f"instinct-{name}.md", command_text)
