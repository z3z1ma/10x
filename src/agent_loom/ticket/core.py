...
def _validate_update_conflicts(
    *,
    parent: Optional[str],
    remove_parent: bool,
    deps: Optional[str],
...
def _apply_frontmatter_updates(
    ...
    *,
    ...
    if status:
        ticket.fm["status"] = require_status(status)
    if priority is not None:
        ticket.fm["priority"] = int(priority)
    if type:
        ticket.fm["type"] = type
    if assignee:
        ticket.fm["assignee"] = assignee
    if tags is not None:
        ticket.fm["tags"] = [x.strip() for x in (tags or "").split(",") if x.strip()]
    if external_ref is not None:
        ticket.fm["external_ref"] = str(external_ref or "").strip()
...
def _apply_parent_update(
    ...
    *,
    ...
    if parent is None:
        return
    parent_value = str(parent or "").strip()
...
def _apply_dependency_update(
    ...
    *,
    ...
    if deps is not None:
        dep_ids = [] if _is_clear_token(str(deps or "")) else _parse_ref_list(store, deps)
    ...
    if not (add_deps or remove_deps):
        return

    current = [str(x) for x in (ticket.fm.get("deps") or [])]
    current_set = {dep for dep in current if dep}
    add_ids = _resolve_refs(store, add_deps)
    remove_ids = _resolve_refs(store, remove_deps)
...
def _apply_incremental_link_update(
    ...
    *,
    ...
    if not (add_links or remove_links):
        return

    add_ids = _resolve_refs(store, add_links)
    remove_ids = _resolve_refs(store, remove_links)
    ...
    for target_id in sorted(set(add_ids + remove_ids)):
        ...
        write_guard(store, target, agent, force=bool(force))
        targets[target_id] = target
        tickets_to_save[target_id] = target

    current_links = {str(link_id) for link_id in (ticket.fm.get("links") or []) if str(link_id)}
    changed_any = False

    for target_id in add_ids:
        other = targets[target_id]
        other_links = {str(link_id) for link_id in (other.fm.get("links") or []) if str(link_id)}
...
def update(
    *,
    ...
    _apply_parent_update(store, t, parent=parent, remove_parent=remove_parent)
    ...
    _apply_link_replace(store, t, links=links)
    if links is None:
    ...
    _apply_body_title_updates(t, body=body, title=title)
