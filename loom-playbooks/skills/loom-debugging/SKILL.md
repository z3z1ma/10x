...
2. the core `loom-evidence` skill when preserving reproduction, red/green output,
   logs, screenshots, or other evidence.

Then read conditionally:

3. the core `loom-research` skill when root cause is unknown or rejected
   hypotheses should remain citable.
4. the core `loom-specs` skill when intended behavior is ambiguous.
5. the core `loom-tickets` skill when creating or tightening the bounded fix
   ticket.
6. the core `loom-ralph` skill when the fix is ready for a packetized
   implementation iteration.
7. the core `loom-critique` skill when the fix or incident carries review risk.
8. the core `loom-retrospective` skill when the prevention lesson should persist
   beyond the ticket.
